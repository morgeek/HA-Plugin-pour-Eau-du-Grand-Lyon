...
class TestLeakAlertSensor:
    ...
    def test_custom_multiplier_from_options(self):
        ...
        s_custom = _make_binary_sensor(EauGrandLyonLeakAlertSensor, data, options={CONF_LEAK_MULTIPLIER: 3.0})
...
class TestLocalLeakSensor:
    def test_alert_when_pattern_detected(self):
        s = _make_binary_sensor(EauGrandLyonLocalLeakSensor, {"contracts": {"REF1": {"local_leak_pattern": True}}})
        assert s.is_on is True

    def test_no_alert_when_pattern_not_detected(self):
        s = _make_binary_sensor(EauGrandLyonLocalLeakSensor, {"contracts": {"REF1": {"local_leak_pattern": False}}})
        assert s.is_on is False
            ...
            EauGrandLyonLocalLeakSensor,
            {
                "contracts": {
                    "REF1": {
                        ...
                        "consommations_journalieres": [{"consommation_m3": 0.1} for _ in range(7)],
                    }
                }
            },
        )
...
class TestBatterySensor:
    def test_alert_when_battery_low(self):
        s = _make_binary_sensor(EauGrandLyonBatterySensor, {"contracts": {"REF1": {"battery_ok": False}}})
        assert s.is_on is True

    def test_no_alert_when_battery_ok(self):
        s = _make_binary_sensor(EauGrandLyonBatterySensor, {"contracts": {"REF1": {"battery_ok": True}}})
        assert s.is_on is False
    ...
    def test_unavailable_when_battery_ok_missing_from_api(self):
        ...
        s = _make_binary_sensor(EauGrandLyonBatterySensor, {"contracts": {"REF1": {}}})
        assert s.available is False
...
class TestLimescaleAlertSensor:
    def test_alert_when_threshold_exceeded(self):
        s = _make_binary_sensor(EauGrandLyonLimescaleAlertSensor, {"contracts": {"REF1": {"limescale_alert": True}}})
        assert s.is_on is True

    def test_no_alert_when_below_threshold(self):
        s = _make_binary_sensor(EauGrandLyonLimescaleAlertSensor, {"contracts": {"REF1": {"limescale_alert": False}}})
        assert s.is_on is False
