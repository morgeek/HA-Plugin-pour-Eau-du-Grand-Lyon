...
class TestDerniereFactureSensor:
    def test_available_when_facture_present(self):
        s = _make(EauGrandLyonDerniereFactureSensor, {"derniere_facture": {"montant_ttc": 42.5}})
    ...
    def test_native_value(self):
        s = _make(EauGrandLyonDerniereFactureSensor, {"derniere_facture": {"montant_ttc": 67.80}})
...
class TestFuiteEstimeeSensor:
    ...
    def test_extra_attrs_capped_14_days(self):
        daily = [{"date": f"2026-01-{i+1:02d}", "volume_fuite_estime_m3": 0.1} for i in range(20)]
        s = _make(
            EauGrandLyonFuiteEstimeeSensor,
            {
